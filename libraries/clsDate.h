...
public:
	...
	static string GetMonthString(short Month)
	{
		if (Month > 12 || Month < 1)
		{
			return "";
		...
		{
			string MonthString[12] = { "Jan","Feb","Mar","Apr","May","jun","jul","Aug","Sept","Oct","Nov","Dec" };
	...
	static string  GetSystemDateAsFormat(string DateFormat)
	{
		...
		DateFormat.replace(DateFormat.find("dd"), 2, to_string(Day));
		DateFormat.replace(DateFormat.find("mm"), 2, to_string(Month));
		DateFormat.replace(DateFormat.find("yyyy"), 4, to_string(Year));
	...
	static	bool IsValidDateTime(clsDate Date)
	{
		...
		if (IsValidDate(Date))
		{
			...
			if (Date.Seconds < 0 || Date.Seconds>60)
			{
				return false;
			}
